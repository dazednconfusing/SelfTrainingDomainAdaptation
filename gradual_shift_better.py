...
def compile_model(model, loss="ce"):
    if loss is not None:
        ...
        model.compile(
            optimizer="adam", loss=[loss], metrics=[metrics.sparse_categorical_accuracy]
        )
    else:
...
def train_model_source(model, split_data, epochs=1000):
    model.fit(
        split_data.src_train_x, split_data.src_train_y, epochs=epochs, verbose=False
    )
...
def run_experiment(
    ...
    def run(seed):
        ...
        target_acc = source_model.evaluate(trg_eval_x, trg_eval_y, return_dict=True)[
            "sparse_categorical_accuracy"
        ]
        # print(
        #     "\n\nevaluated source model on target:",
        #     source_model.evaluate(trg_val_x, trg_val_y),
        # )
        ...
        acc = student.evaluate(trg_eval_x, trg_eval_y, return_dict=True)[
            "sparse_categorical_accuracy"
        ]
...
def experiment_results(save_name):
    ...
    print(
        "\nNon-adaptive accuracy on source (%): ",
        np.mean(src_accs),
        mult * np.std(src_accs) / np.sqrt(num_runs),
    )
    print(
        "Non-adaptive accuracy on target (%): ",
        np.mean(target_accs),
        mult * np.std(target_accs) / np.sqrt(num_runs),
    )
    print(
        "Gradual self-train accuracy (%): ",
        np.mean(final_graduals),
        mult * np.std(final_graduals) / np.sqrt(num_runs),
    )
    print(
        "Target self-train accuracy (%): ",
        np.mean(final_targets),
        mult * np.std(final_targets) / np.sqrt(num_runs),
    )
    print(
        "All self-train accuracy (%): ",
        np.mean(final_alls),
        mult * np.std(final_alls) / np.sqrt(num_runs),
    )
    print(
        "Best of Target self-train accuracies (%): ",
        np.mean(best_targets),
        mult * np.std(best_targets) / np.sqrt(num_runs),
    )
    print(
        "Best of All self-train accuracies (%): ",
        np.mean(best_alls),
...
def rotated_mnist_60_conv_experiment():
    run_experiment(
        dataset_func=datasets.rotated_mnist_60_data_func,
        n_classes=10,
        input_shape=(28, 28, 1),
...
def rotated_mnist_60_vae_experiment():
    run_experiment(
        dataset_func=datasets.rotated_mnist_60_data_func,
        n_classes=10,
        input_shape=(28, 28, 1),
...
def portraits_conv_experiment():
    run_experiment(
        dataset_func=datasets.portraits_data_func,
        n_classes=2,
        input_shape=(32, 32, 1),
...
def gaussian_linear_experiment():
    d = 100
    run_experiment(
        dataset_func=lambda: datasets.gaussian_data_func(d),
        n_classes=2,
        input_shape=(d,),
        save_file="saved_files/gaussian.dat",
        model_func=models.linear_softmax_model,
        interval=500,
        epochs=100,
        loss="ce",
        soft=False,
...
def portraits_64_conv_experiment():
    run_experiment(
        dataset_func=datasets.portraits_64_data_func,
        n_classes=2,
        input_shape=(64, 64, 1),
...
def portraits_conv_experiment_more():
    run_experiment(
        dataset_func=datasets.portraits_data_func_more,
        n_classes=2,
        input_shape=(32, 32, 1),
...
if __name__ == "__main__":
    ...
    rotated_mnist_60_vae_experiment()
    print("Rot MNIST vae experiment")
    experiment_results("saved_files/rot_mnist_60_vae.dat")
