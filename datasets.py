...
def get_split_data(dataset):
    ...
    inter_x, inter_y = (
        Xs[n_src : dataset.target_end - n_target],
...
def sample_gaussian_alpha(
    source_means, source_sigmas, target_means, target_sigmas, alpha_low, alpha_high, N
):
...
def make_high_d_gaussian_data(
    ...
    return make_moving_gaussian_data(
        source_means=[means[0], means[1]],
        source_sigmas=[var_list[0], var_list[1]],
        target_means=[means[2], means[3]],
        target_sigmas=[var_list[2], var_list[3]],
        source_alphas=source_alphas,
        inter_alphas=inter_alphas,
...
def make_moving_gaussians(
    source_means, source_sigmas, target_means, target_sigmas, steps
):
    source_means = shape_means(source_means)
    target_means = shape_means(target_means)
    source_sigmas = shape_sigmas(source_sigmas, source_means)
...
def high_d_gaussians(d, var, n):
    ...
    t_a = -2 * perp - v
    t_b = 2 * perp + v
    return lambda: make_moving_gaussians(
        [s_a, s_b], [var, var], [t_a, t_b], [var, var], n
    )
...
def _transition_rotation_dataset(
    train_x,
    train_y,
    test_x,
    test_y,
...
def dial_proportions_rotated_dataset(
    train_x,
    train_y,
    test_x,
    test_y,
    ...
    inter_func = lambda x: dial_rotation_proportions(x, source_angles, target_angles)
...
def make_rotated_dataset(
    ...
    inter_func = lambda x: continually_rotate_images(
        x, inter_angles[0], inter_angles[1]
    )
    return _transition_rotation_dataset(
        train_x,
        train_y,
...
def save_data(
    data_dir="dataset_32x32", save_file="dataset_32x32.mat", target_size=(32, 32)
):
    Xs, Ys = [], []
...
def portraits_64_data_func():
    return make_portraits_data(
        1000, 1000, 14000, 2000, 1000, 1000, load_file="dataset_64x64.mat"
    )
...
def gaussian_data_func(d):
    return make_high_d_gaussian_data(
        ...
        min_var=0.05,
        max_var=0.1,
        source_alphas=[0.0, 0.0],
        inter_alphas=[0.0, 1.0],
