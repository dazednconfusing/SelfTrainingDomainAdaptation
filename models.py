...
def vae_model(num_labels, input_shape):
    return VAE(num_labels, input_shape)
...
def mlp_softmax_model(num_labels, input_shape, l2_reg=0.02):
    linear_model = keras.models.Sequential(
        [
            keras.layers.Flatten(input_shape=input_shape),
            keras.layers.Dense(
                32, activation=tf.nn.relu, kernel_regularizer=regularizers.l2(0.0)
            ),
            keras.layers.Dense(
                32, activation=tf.nn.relu, kernel_regularizer=regularizers.l2(0.0)
            ),
...
def simple_softmax_conv_model(
    num_labels, hidden_nodes=32, input_shape=(28, 28, 1), l2_reg=0.0
):
    return keras.models.Sequential(
        [
            ...
            keras.layers.Conv2D(
                hidden_nodes, (5, 5), (2, 2), activation=tf.nn.relu, padding="same"
            ),
            keras.layers.Conv2D(
                hidden_nodes, (5, 5), (2, 2), activation=tf.nn.relu, padding="same"
            ),
...
def unregularized_softmax_conv_model(
    num_labels, hidden_nodes=32, input_shape=(28, 28, 1), l2_reg=0.0
):
    return keras.models.Sequential(
        [
...
def papernot_softmax_model(num_labels, input_shape=(28, 28, 1), l2_reg=0.0):
    papernot_conv_model = keras.models.Sequential(
        [
            ...
            keras.layers.Conv2D(
                128, (6, 6), (2, 2), activation=tf.nn.relu, padding="valid"
            ),
            keras.layers.Conv2D(
                128, (5, 5), (1, 1), activation=tf.nn.relu, padding="valid"
            ),
            keras.layers.BatchNormalization(),
            keras.layers.Flatten(name="after_flatten"),
            keras.layers.Dense(num_labels, activation=tf.nn.softmax, name="out"),
